...
void add_variable_parameter(const char* c_vehicle_name, const char* parameter_name, const int n, const double* s, const double* values)
{
    ...
    if ( vehicles_limebeer2014f1.count(vehicle_name) != 0 )
    {
        vehicles_limebeer2014f1.at(vehicle_name).add_variable_parameter(std::string(parameter_name), p);
    }
    else if ( vehicles_lot2016kart.count(vehicle_name) != 0)
    {
        vehicles_lot2016kart.at(vehicle_name).add_variable_parameter(std::string(parameter_name), p);
    }
    else
        throw std::runtime_error("Vehicle type not recognized");
}
